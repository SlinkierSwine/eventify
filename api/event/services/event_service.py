...
class EventService:
    ...
    def create_event_participants(
        cls, event: Event, participants: Iterable[AbstractBaseUser | AnonymousParticipant]
    ):
        for participant in participants:
            ...
            if isinstance(participant, AbstractBaseUser):
                ...
                if not created:
                    raise EventException(f"Participant with {participant.notification_provider} ID = {participant.social_contact} is already participating in {event}")
